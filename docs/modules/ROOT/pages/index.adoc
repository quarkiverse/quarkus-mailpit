...
Mailpit provides chaos testing capabilities that allow you to simulate various email delivery scenarios. This is particularly useful for testing how your application handles email delivery failures.

You can configure chaos testing through the Mailpit API. The extension provides a convenient way to set up chaos testing in your tests:
...
public class MailpitChaosTest {
    ...
    public void testChaos() throws ApiException {
        ...
        mailbox.setChaos(ChaosConfig.builder()
            .acceptRate(50)     // 50% of emails will be accepted
            .rejectMsg("Custom rejection message")
            .build());

        // Your email sending test code here...
    }

    @AfterEach
    public void cleanup() throws ApiException {
        // Don't forget to disable chaos after the test
...
* `acceptRate` - Percentage of emails to accept (0-100)
* `rejectMsg` - Custom SMTP rejection message
* `disconnect` - Randomly disconnect SMTP connections
* `delay` - Add random SMTP processing delays (in seconds)
